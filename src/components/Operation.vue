<template>
  <div class="operation">
    ...
    <div class="text-alignment">
        <div v-for="item in AlignList" :key="item.id" class="item" @click="(item.align)(elsIdx, useElStore.rectValue)">
...
export default {
  ...
  setup() {
    ...
    function rectSynchronous(rectValue) {
      if (useElStore.els[useElStore.elsIdx[0]][useElStore.elsIdx[1]] === undefined) {
        useElStore.rectIsShow = false;
      } else {
        useElStore.rectStyle.style.left = useElStore.els[useElStore.elsIdx[0]][useElStore.elsIdx[1]].style.left;
        useElStore.rectStyle.style.top = useElStore.els[useElStore.elsIdx[0]][useElStore.elsIdx[1]].style.top;
        useElStore.rectStyle.style.width = useElStore.els[useElStore.elsIdx[0]][useElStore.elsIdx[1]].style.width;
        useElStore.rectStyle.height = useElStore.els[useElStore.elsIdx[0]][useElStore.elsIdx[1]].style.height;
        useElStore.rectStyle.style.rotate = useElStore.els[useElStore.elsIdx[0]][useElStore.elsIdx[1]].style.rotate;
      }
    }
    ...
    function alignCenter(elsIdx, rectValue) {
      if (useElStore.rectIsShow) {
          console.log(useElStore.els[elsIdx[0]][elsIdx[1]].style)
        els[elsIdx[0]][elsIdx[1]].style.left = 876 / 2 - els[elsIdx[0]][elsIdx[1]].style.width / 2;
        useElStore.els = els; 
        rectSynchronous(rectValue); 
        //居中对齐时，添加新的快照
        useSnapshot.recordSnapshot();
    ...
    function alignRight(elsIdx, rectValue) {
      if (useElStore.rectIsShow) {
        console.log(useElStore.els[elsIdx[0]][elsIdx[1]].style)
        els[elsIdx[0]][elsIdx[1]].style.left = 876 - els[elsIdx[0]][elsIdx[1]].style.width;
        useElStore.els = els;
        rectSynchronous(rectValue); 
        //居右对齐时，添加新的快照
        useSnapshot.recordSnapshot();
